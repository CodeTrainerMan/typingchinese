...
内置 450 个词，分五本：**日常常用词**（59）、**进阶词汇**（51）、**四字成语**（40）、**HSK 一级**（150）、**HSK 二级**（150）。
...
本项目已经接好 GitHub → Vercel 的自动部署：**推送到 `main` 即触发生产部署，不需要手动执行部署命令**。
换句话说，日常流程只有 `git commit` + `git push`，部署状态去 Vercel 面板看。

## 开发

工程约定、已知风险与测试说明见 [`DEV.md`](./DEV.md)。
